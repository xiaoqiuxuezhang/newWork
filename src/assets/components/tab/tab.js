define('components/tab/tab.tpl',['hdbr'], function(Handlebars) {
			return Handlebars.template({"compiler":[7,">= 4.0.0"],"main":function(container,depth0,helpers,partials,data) {
    return "<div class=\"sn-tab-container\">\r\n    <div class=\"sn-tab J_tab_render\">\r\n        <ul class=\"sn-tab-items\">\r\n        </ul>\r\n        <div class=\"sn-dropdown\"></div>\r\n        <div class=\"contentArea J_content_render\"></div>\r\n    </div>\r\n</div>";
},"useData":true})
		});
define('components/tab/tabItem.tpl',['hdbr'], function(Handlebars) {
  ...
  return "        <span class=\""
    ...
    return "    <span class=\"sn-tabClose\"></span>\r\n";
},"compiler":[7,">= 4.0.0"],"main":function(container,depth0,helpers,partials,data) {
    var stack1, helper, alias1=depth0 != null ? depth0 : (container.nullContext || {});

  return "<li class=\"J_item_click\">\r\n    <span class=\"iconBox\">\r\n"
    ...
    + "\r\n    </span>\r\n"
    + ((stack1 = helpers["if"].call(alias1,(depth0 != null ? depth0.closeable : depth0),{"name":"if","hash":{},"fn":container.program(3, data, 0),"inverse":container.noop,"data":data})) != null ? stack1 : "")
    + "</li>";
},"useData":true})
		});
...
define('tab',[
	...
	'components/tab/tab.tpl',
	'components/tab/tabItem.tpl',
	'lib/requirejs/css.min!components/tab/tab.css'
], function($, Hdb, EventTarget, tpl, template) {
	var VERSION = '1.1.0';
//	var template = Hdb.compile(itemTpl);
	...
	var render = function() {
		...
		for(var i = 0; i < tabDatas.length; i++) {
			...
			$tpl.find('.sn-tab>.sn-tab-items').append(template(tabDatas[i]));
			$tpl.find('.sn-tab>.sn-tab-items>li:last').addClass(className);
		}
		this.$el.html($tpl);
		if(this.options.direction && this.options.direction === "vertical") {
			this.$el.addClass("sn-vertical");
			this.$el.find('.sn-tab .sn-tab-items > li').css({padding: 0});
		}
		if(this.options.className) {
			this.$el.find('>.sn-tab-container').addClass(this.options.className);
		}
		//多行配置项
		if(this.options.multiline && this.options.multiline === true && this.options.direction !== 'vertical') {
			multilineHanlerInit.call(this);
		}
	};
	var multilineHanlerInit = function() {
		...
		if(ulHeight > liHeight) {
			...
			if(!$dropdownBtn.hasClass("sn-btnclose")) {
				$ul.css('padding-right', '0');
				$dropdownBtn.hide();
			}
		}
		$dropdownBtn.off().on('click', function() {
			if($dropdownBtn.hasClass("sn-btnclose")) {
				$ul.css('height', 'auto');
				$ul.css('overflow', 'visiable');
		...
		//		})
		//		var $ul = this.$el.find('>.sn-tab-container>.J_tab_render>.sn-tab-items');
	...
	var eventInit = function() {
		this.$el.off('click', itemSelectStr);
		// 最初写法，不作任何处理
	...
	$.extend(objClass.prototype, EventTarget.prototype, {
		...
		destroy: function(title, e) {
			...
			if(_self.$el.find(itemSelectStr).length > 1) {
				...
				$.each(this.tabItem, function(key, val) {
					if(key == title) {
						$li = _self.$el.find('.sn-tab-container>.sn-tab>.sn-tab-items>.' + title);
						return false;
					} else if(val == title) {
						$li = _self.$el.find('li:contains(' + title + ')');
						return false;
				...
				if($li) {
					...
					if(index != -1) {
						...
						if(_self.$el.find('.active').length == 0) {
							var $newLi = _self.$el.find('.sn-tab-container>.sn-tab>.sn-tab-items>.J_item_click:eq(' + index + ')');
							if($newLi.length == 0) {
								_self.$el.find('.sn-tab-container>.sn-tab>.sn-tab-items>.J_item_click:eq(' + (index - 1) + ')').trigger("click");
							} else {
								$newLi.trigger("click");
							}
						}
		...
		setTitle: function(className, title) {
			...
			for(var i = 0; i < this.options.tabs.length; i++) {
				if(this.options.tabs[i].className == className) {
					...
					this.$el.find('.sn-tab-container>.sn-tab>.sn-tab-items>.J_item_click.' + className).find('a').text(title);
				}
			}
		},
		//创建选项卡
		createTab: function(title, render, param) {
			...
			this.$el.find('.sn-tab-container>.sn-tab>.sn-tab-items').append(template(this.options.tabs[length - 1]));
			var $liLast = this.$el.find('.sn-tab-container>.sn-tab>.sn-tab-items>li:last');
		...
		content: function(html) {
			var index = this.$el.find(".sn-tab-container>.sn-tab>.sn-tab-items").find('.active').index();
	...
	var sizeInit = function() {
		if(!(this.options.direction && this.options.direction === "vertical") && this.options.fixedNav && this.$el.height()) {
			...
			var navHeight = this.$el.find('>.sn-tab-container>.J_tab_render>.sn-tab-items').height();
