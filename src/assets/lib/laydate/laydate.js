...
;!function(){
  ...
  */
  ...
  Class.prototype.init = function(){
    ...
    //若今日时间不符合最大最小值限制，则禁用按钮
    var nowTimes=this.systemDate(),maxTime=that.config.max,minTime=that.config.min,maxx,minn,noww;
    noww=nowTimes.year+that.checkTime(nowTimes.month+1)+that.checkTime(nowTimes.date)+that.checkTime(nowTimes.hours)+that.checkTime(nowTimes.minutes)+that.checkTime(nowTimes.seconds);
    maxx=maxTime.year+that.checkTime(maxTime.month+1)+that.checkTime(maxTime.date)+that.checkTime(maxTime.hours)+that.checkTime(maxTime.minutes)+that.checkTime(maxTime.seconds);
    minn=minTime.year+that.checkTime(minTime.month+1)+that.checkTime(minTime.date)+that.checkTime(minTime.hours)+that.checkTime(minTime.minutes)+that.checkTime(minTime.seconds);
    if(noww>maxx||noww<minn){
      lay(that.footer).find(".laydate-btns-now").addClass(DISABLED);
    }
  ...
  Class.prototype.checkTime=function(ss){
    if(ss<10){
      return "0"+ss;
    }else{
      return ss+"";
    ...
    if(type === 'year'){ //年列表
      ...
      lay.each(new Array(15), function(i){
        ...
        if(yearNum < that.firstDate.year){
          ...
          ymd.month = options.dateTime.month;
          ymd.date = options.dateTime.date;
        }
